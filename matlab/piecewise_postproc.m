function piecewise_postproc(t,z)
global gv
...
for ii=1:npie
    ...
    plot(t,p(:,ii))
    grid on
    title(strcat('tras y strainof piece',num2str(ii)))
    xlabel('t [s]')
    ylabel('p [-]')
...
mov               =VideoWriter(strcat('.\LAST RUN\Section Dynamics2'),'MPEG-4');
...
%     axis ([-npie*L npie*L 0 1.5*npie*L -L L])  % cantilever
    ...
    % set the graph options
    set(gca,'CameraPosition',[0.5*npie*L 0 0],...
        'CameraTarget',[0 0 0],...
...
%     axis ([-npie*L npie*L 0 1.5*npie*L -L L])  % cantilever
    ...
    % set the graph options
    set(gca,'CameraPosition',[0 0 -0.5*npie*L],...
        'CameraTarget',[0 0 0],...
...
%     axis ([-npie*L npie*L 0 1.5*npie*L -L L])  % cantilever
    ...
    % set the graph options
    set(gca,'CameraPosition',[0 0.5*npie*L 0],...
        'CameraTarget',[0 0 0],...
        'CameraUpVector',[1 0 0])
    axis equal
    grid on
