...
time        = 7;                     % [s]
nsol        =time*10^3+1;            % one solution every centisecond
...
% qd = @(t) repmat([0;0;0;1;0;delta], npie, 1);
% qd_dot = @(t) zeros(6*npie, 1);
% qd_ddot = @(t) zeros(6*npie, 1);

% specify the controller gain
Kp = 50*eye(6*npie);
Kv = 0.1*eye(6*npie);

opt = 'backstepping';

cl_derivs = @(t, z) closed_loop_derivatives(t, z, qd, qd_dot, qd_ddot, Kp, Kv, opt);
...
disp('Time-advancing')
myopt          =odeset('RelTol',1e-4,'OutputFcn',@save_ode_data);
...
% integrate
[t,z]          =ode45(cl_derivs,tspan,ini_cond,myopt);
