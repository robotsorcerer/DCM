...
class _UncheckedAssign(torch.autograd.Function):
    ...
    def forward(ctx, scratch, value, index):
        ctx.index = index
        # print(f"scratch.data {scratch.data[index].shape}, value: {value.shape}, index: {index}")
...
def _runge_kutta_step(func, y0, f0, t0, dt, t1, tableau):
    ...
    for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)):
        ...
        t_loc, f = func(ti, yi, perturb=perturb)
...
def rk4_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):
    if k1 is None:
        k1 = func(t0, y0, perturb=Perturb.NEXT if perturb else Perturb.NONE)[1]
    else:
        k1 = f0[1] if isinstance(f0, tuple) else f0
    half_dt = dt * 0.5
    k2 = func(t0 + half_dt, y0 + half_dt * k1)[1]
    k3 = func(t0 + half_dt, y0 + half_dt * k2)[1]
    k4 = func(t1, y0 + dt * k3, perturb=Perturb.PREV if perturb else Perturb.NONE)[1]
...
def rk4_alt_step_func(func, t0, dt, t1, y0, f0=None, perturb=False):
    ...
    if k1 is None:
        ...
        k1 = f0[1] if isinstance(f0, tuple) else f0
    k2 = func(t0 + dt * _one_third, y0 + dt * k1 * _one_third)[1]
    k3 = func(t0 + dt * _two_thirds, y0 + dt * (k2 - k1 * _one_third))[1]
    k4 = func(t1, y0 + dt * (k1 - k2 + k3), perturb=Perturb.PREV if perturb else Perturb.NONE)[1]
